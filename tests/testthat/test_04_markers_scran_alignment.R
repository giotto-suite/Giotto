# Cluster labels have to reach `scran::findMarkers()` in the expression matrix's
# column order: it pairs them positionally, and expression and cell metadata are
# fetched independently with no guarantee they share a cell order.
#
# On suite_dev this was a reorder inside findScranMarkers(). Here the resolution
# lives in `.markers_scran()`, which `findScranMarkers()` reaches through
# `analyzeData(x, markersParam(method = "scran"))` -- so these assert the public
# behaviour and are indifferent to which layer supplies it.
